...
@Directive({
  ...
  selector: '[dynamicevents]'
})
export class EventsDynamicDirective implements OnInit, OnDestroy, AfterViewInit {
  @Input() dynamicevents: Observable<any>;
  data: Subscription;
  ...
  ngAfterViewInit() {
    this.dynamicevents.subscribe((data) => {
      if (data) {
        console.log(data);
