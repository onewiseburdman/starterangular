...
@Directive({
  ...
  selector: '[dynamicregister]'
})
export class RegisterDynamicDirective implements OnInit, OnDestroy, AfterViewInit {
  @Input() dynamicregister: Observable<any>;
  data: Subscription;
  ...
  ngAfterViewInit() {
    this.dynamicregister.subscribe((data) => {
      if (data) {
        console.log(data);
