...
@Directive({
  ...
  selector: '[dynamiccontact]'
})
export class ContactDynamicDirective implements OnInit, OnDestroy, AfterViewInit {
  @Input() dynamiccontact: Observable<any>;
  data: Subscription;
  ...
  ngAfterViewInit() {
    this.dynamiccontact.subscribe((data) => {
      if (data) {
        console.log(data);
